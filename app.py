...
# Encrypt function with custom or default key
def encrypt_file(file_data, custom_key=None):
    if custom_key:
        # Check if custom_key is a string and encode if necessary
        if isinstance(custom_key, str):
            custom_key = base64.urlsafe_b64encode(custom_key.ljust(32)[:32].encode())
        elif isinstance(custom_key, bytes):
            custom_key = base64.urlsafe_b64encode(custom_key.ljust(32)[:32])
    else:
        # Use the default key
...
# Decrypt function with provided key
...
def upload_file():
    if 'user' in session:
        ...
        # Encrypt file data
        file_data = file.read()
        encrypted_data, fernet = encrypt_file(file_data, encryption_key.encode() if encryption_key else None)
        with open(file_path, 'wb') as f:
            f.write(encrypted_data)
        
        # Save key if 'custom' was chosen
        if encrypt_option == 'custom':
            session['custom_key'] = encryption_key
            ...
            return '''
                ...
                <body>
                    <h3 style="font-size:20px; color:green; text-align:center;">File uploaded and encrypted successfully!</h3>
                </body>
                </html>
            '''
    return redirect(url_for('login'))
